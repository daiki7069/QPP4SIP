...
uv run python run.py --mode inference --model music --input_path $INPUT_PATH/bm25_dev.pkl --saved_model_path $CHECKPOINT_PATH --epoch_num $EPOCH_NUM > $LOG_PATH/inference.log 2>&1
if [ $? -eq 0 ]; then
...
uv run python run.py --mode evaluation --model music --input_path $INPUT_PATH/bm25_dev.pkl --output_path $OUTPUT_PATH/result --epoch_num $EPOCH_NUM > $LOG_PATH/evaluation.log 2>&1
if [ $? -eq 0 ]; then
