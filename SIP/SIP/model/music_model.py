...
class crf(nn.Module):
    ...
    def __init__(self, args=None):
        ...
        self.matrice_u2s_I0 = nn.Parameter(torch.randn(2, 2) * 0.1)  # no initiave-taking before
        self.matrice_u2s_I1 = nn.Parameter(torch.randn(2, 2) * 0.1)  # 1 system's initiave-taking before
        self.matrice_u2s_I2_more = nn.Parameter(torch.randn(2, 2) * 0.1)  # 2 and more

        # the distance from the last system's initiative-taking to the current system's turn
        self.matrice_u2s_Id2 = nn.Parameter(torch.randn(2, 2) * 0.1)  # consecutive  e.g., 4-2=2
        self.matrice_u2s_Id4_more = nn.Parameter(torch.randn(2, 2) * 0.1)  # inconsecutive e.g., 6-2=4 and  8-2=6

        self.matrice_1_2 = nn.Parameter(torch.randn(2, 2) * 0.1)  # u2s
        self.matrice_2_3 = nn.Parameter(torch.randn(2, 2) * 0.1)  # s2u
        self.matrice_3_4 = nn.Parameter(torch.randn(2, 2) * 0.1)  # u2s
        self.matrice_4_5 = nn.Parameter(torch.randn(2, 2) * 0.1)  # s2u
        self.matrice_5_6 = nn.Parameter(torch.randn(2, 2) * 0.1)  # u2s
        self.matrice_6_7 = nn.Parameter(torch.randn(2, 2) * 0.1)  # s2u
        self.matrice_7_8 = nn.Parameter(torch.randn(2, 2) * 0.1)  # u2s
        self.matrice_8_9 = nn.Parameter(torch.randn(2, 2) * 0.1)  # s2u
        self.matrice_9_10 = nn.Parameter(torch.randn(2, 2) * 0.1)  # u2s
        self.matrice_10_11 = nn.Parameter(torch.randn(2, 2) * 0.1)  # s2u
        self.matrice_11_12 = nn.Parameter(torch.randn(2, 2) * 0.1)  # u2s
        self.matrice_12_13 = nn.Parameter(torch.randn(2, 2) * 0.1)  # s2u
        self.matrice_13_14 = nn.Parameter(torch.randn(2, 2) * 0.1)  # u2s
        self.matrice_14_15 = nn.Parameter(torch.randn(2, 2) * 0.1)  # s2u
        self.matrice_15_16 = nn.Parameter(torch.randn(2, 2) * 0.1)  # u2s
        self.matrice_16_17 = nn.Parameter(torch.randn(2, 2) * 0.1)  # s2u
        self.matrice_17_18 = nn.Parameter(torch.randn(2, 2) * 0.1)  # u2s
        self.matrice_18_19 = nn.Parameter(torch.randn(2, 2) * 0.1)  # s2u
        self.matrice_19_20 = nn.Parameter(torch.randn(2, 2) * 0.1)  # u2s
...
class ContextEncoding(nn.Module):
    """
    Context Encoding with BERT
    Encode the entire context up to the current turn.
    """
    def __init__(self, args=None):
        super().__init__()
        self.args = args
...
class OneStep(nn.Module):
    """
    One step GRU for prior network
    Predict the current turn's I label based on the context representation of the current turn and the previous state.
    """
    def __init__(self, args=None, action_embedding=None):
        super().__init__()
        self.args = args
    ...
    def forward(self, tgt, state, feature):
        ...
        if self.action_embedding is not None:
            embedded = self.action_embedding(tgt) # [pair_num, 1, hidden_size]
            input = torch.cat([embedded, feature], 2) # [pair_num, 1, hidden_size*2]
        else:
            input = feature # [pair_num, 1, hidden_size]
