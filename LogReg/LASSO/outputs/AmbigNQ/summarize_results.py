...
def create_heatmap(df):
    ...
    # メトリクスを抽出
    metrics = ['accuracy', 'f1', 'auc', 'ap']
    heatmap_data = df[['experiment'] + metrics].set_index('experiment')
...
def create_summary_report(df):
    ...
    for _, row in df.iterrows():
        ...
        report.append(f"  F1 Score: {row['f1']:.4f}")
        report.append(f"  AUC-ROC: {row['auc']:.4f}")
        report.append(f"  Average Precision: {row['ap']:.4f}")
