#!/usr/bin/env python3
"""
AmbigQAデータセットをINSCIT形式のbm25_dev.json/bm25_train.jsonに変換するスクリプト
"""
...
def load_bm25_predictions(predictions_file: str) -> List[Dict[str, Any]]:
    """
    BM25のpredictionデータを読み込む（idとscoreを含む）
    
    Args:
        predictions_file: dev_predictions.jsonまたはtrain_predictions.jsonのパス
        
    Returns:
        各質問に対応する{passage_ids: List[int], scores: List[float]}のリスト
    """
    print(f"[INFO] Loading BM25 predictions from {predictions_file}...")
    with open(predictions_file, 'r', encoding='utf-8') as f:
        predictions = json.load(f)
    
    print(f"[INFO] Loaded BM25 predictions for {len(predictions)} questions")
    return predictions
...
def create_inscit_format(
    ...
    Args:
        questions: 質問データのリスト
        bm25_predictions: 各質問に対応する{passage_ids: List[int], scores: List[float]}のリスト
    ...
    for i, question in enumerate(tqdm(questions, desc="Converting to INSCIT format", unit="questions")):
        ...
        # BM25予測データからpassage IDとscoreを取得
        # nqopenのインデックスマッピングがある場合はそれを使用、なければインデックスで直接アクセス
        if nqopen_id_to_index is not None and q_id in nqopen_id_to_index:
            bm25_index = nqopen_id_to_index[q_id]
            if bm25_index >= len(bm25_predictions):
                print(f"[WARN] BM25 index {bm25_index} out of range for question {q_id}")
                missing_bm25_predictions += 1
                continue
            bm25_pred = bm25_predictions[bm25_index]
        elif len(bm25_predictions) > len(questions):
            # インデックスマッピングがない場合、最初のN件を使用（警告を出す）
            if i == 0:
                print(f"[WARN] No nqopen index mapping provided, using first {len(questions)} BM25 predictions")
            if i >= len(bm25_predictions):
                print(f"[WARN] BM25 index {i} out of range for question {q_id}")
                missing_bm25_predictions += 1
                continue
            bm25_pred = bm25_predictions[i]
        else:
            # 長さが一致している場合
            if i >= len(bm25_predictions):
                print(f"[WARN] BM25 index {i} out of range for question {q_id}")
                ...
                continue
            bm25_pred = bm25_predictions[i]
        passage_ids = bm25_pred.get("passage_ids", [])[:top_k]
        scores = bm25_pred.get("scores", [])[:top_k]
        ...
        for passage_id, score in zip(passage_ids, scores):
            ...
            if passage_id in passages:
                ...
                ctx = {
                    ...
                    "title": passage["title"],
                    "text": passage["text"],
                    "score": str(score),  # BM25から取得したスコアを使用
    ...
    if missing_bm25_predictions > 0:
        print(f"  - Missing BM25 predictions: {missing_bm25_predictions}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert AmbigQA dataset to INSCIT format (bm25_dev.json/bm25_train.json)"
    )
    parser.add_argument(
        "--split",
        required=True,
    ...
    # BM25予測ファイルのパス
    if args.split == "dev":
        bm25_predictions_file = Path("/mnt/nas_syno/daiki/Datasets/AmbigQA/codes/out/bm25/dev_predictions.json")
        nqopen_file = Path("/mnt/nas_syno/daiki/Datasets/AmbigQA/codes/data/nqopen/dev.json")
    elif args.split == "train":
        bm25_predictions_file = Path("/mnt/nas_syno/daiki/Datasets/AmbigQA/codes/out/bm25/train_predictions.json")
    ...
    if not bm25_predictions_file.exists():
        raise FileNotFoundError(f"BM25 predictions file not found: {bm25_predictions_file}")
    ...
    bm25_predictions = load_bm25_predictions(str(bm25_predictions_file))
    ...
    # INSCIT形式に変換
    inscit_data = create_inscit_format(questions, bm25_predictions, passages, nqopen_id_to_index=nqopen_id_to_index, top_k=args.top_k)
    
    # 出力ファイル名
    output_file = output_dir / f"bm25_{args.split}.json"
